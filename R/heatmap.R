...
debrowserheatmap <- function(id, expdata = NULL) {
  ...
  hselGenes <- reactive({
    if (is.null(input$selgenenames)) {
      return("")
    }
    unlist(strsplit(input$selgenenames, split = ","))
  })
  shg <- reactive({
    if (is.null(input$hoveredgenename)) {
      return("")
    }
...
runHeatmap <- function(input = NULL, session = NULL, expdata = NULL) {
  ...
  if (!input$customColors) {
    heatmapColors <- eval(parse(text = paste0(
      input$pal,
      "(", input$ncol, ")"
    )))
  } else {
    if (!is.null(input$color1)) {
      heatmapColors <- colorRampPalette(c(
        input$color1,
        input$color2, input$color3
      ))(n = 1000)
    }
  }
...
runHeatmap2 <- function(input = NULL, session = NULL, expdata = NULL) {
  ...
  if (input$kmeansControl) {
    m <- heatmap.2(as.matrix(expdata),
      Rowv = FALSE, main = input$main, dendrogram = input$dendrogram,
    ...
    m <- heatmap.2(as.matrix(expdata),
      main = input$main, dendrogram = input$dendrogram,
...
clustFunParamsUI <- function() {
  c(
    Complete = "complete", Single = "single", Average = "average",
...
distFunParamsUI <- function() {
  c(
    Cor = "cor", Euclidean = "euclidean", Maximum = "maximum",
