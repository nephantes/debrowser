...
deUI <- function() {
  ...
  debrowser <- (fluidPage(
    ...
    # Loading message
    tags$div(h4(paste0("Loading DEBrowser v", getNamespaceVersion("debrowser"))),
      id = "loading-debrowser",
      tags$img(src = "www/images/initial_loading.gif")
    ),
    tags$head(
      tags$title(paste0("DEBrowser v", getNamespaceVersion("debrowser"))),
      tags$link(
    ...
    dashboardPage(
      ...
      dashboardSidebar(
        ...
        uiOutput("loading"),
        tabsetPanel(
          id = "menutabs", type = "tabs",
          tabPanel(
            title = "Data Prep", value = "dataprep", id = "dataprep",
            sidebarMenu(
              id = "DataPrep",
              menuItem("Quick Start Guide",
                icon = icon("user"),
          ...
          tabPanel(
            title = "Discover", value = "discover", id = "discover",
            conditionalPanel(
      ...
      dashboardBody(
        mainPanel(
          ...
          tabsetPanel(
            id = "methodtabs", type = "tabs",
            tabPanel(
              title = "Data Prep", value = "panel0", id = "panel0",
              tabItems(
            ...
            tabPanel(
              title = "Main Plots", value = "panel1", id = "panel1",
              uiOutput("mainmsgs"),
              uiOutput("mainpanel")
            ),
            tabPanel(
              title = "QC Plots", value = "panel2", id = "panel2",
              uiOutput("qcpanel")
            ),
            tabPanel(
              title = "GO Term", value = "panel3", id = "panel3",
              uiOutput("gopanel")
            ),
            tabPanel(
              title = "Tables", value = "panel4", id = "panel4",
