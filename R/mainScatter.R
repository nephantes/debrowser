...
debrowsermainplot <- function(id, data = NULL, cond_names = NULL) {
  ...
  output$mainPlotControlsUI <- renderUI({
    ...
    list(
      ...
      conditionalPanel(
        paste0("input['", session$ns("labelsearched"), "']"),
  ...
  output$volcanoControlsUI <- renderUI({
    ...
    list(
      ...
      conditionalPanel(
        condition <- paste0("input['", session$ns("limitPadj"), "']"),
...
mainScatterNew <- function(input = NULL, data = NULL, cond_names = NULL, source = NULL) {
  ...
  p <- plot_ly(
    source = source, data = data, x = ~x, y = ~y, key = ~key, alpha = 0.8,
...
generateTestData <- function(dat = NULL) {
  ...
  params <-
    ...
    c("DESeq2", "NoCovariate", "parametric", FALSE, "LRT", "None")
  ...
  rdata <- cbind(
    rownames(de_res), norm_data[rownames(de_res), columns],
    log10(rowMeans(norm_data[rownames(de_res), cols1])
    + 0.1), log10(rowMeans(norm_data[rownames(de_res), cols2])
    + 0.1), de_res[
      rownames(de_res),
      c("padj", "log2FoldChange")
    ], 2^de_res[rownames(de_res), "log2FoldChange"], -1 *
