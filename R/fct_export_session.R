...
#'
#' One element per line. Same blocks as [emit_r_script()] but wrapped in
#' an Rmd structure: YAML front matter, "## Methods" paragraph (assembled
#' from `methods_sentences()`), "## Pipeline" chunks (`eval = FALSE` by
#' default), final "## Session info" chunk (`eval = TRUE`). Caller renders
#' to HTML via `rmarkdown::render()`.
#'
#' @param blocks Output of [build_session_blocks()].
#' @return character vector.
#' @keywords internal
...
emit_rmd <- function(blocks) {
  ...
  batch_chunk <- if (identical(blocks$batch$method, "none")) {
    c("### 3. Batch correction", "", "*No batch correction was applied.*", "")
  } else {
    treat_arg <- if (is.na(blocks$batch$treatment_column %||% NA_character_)) {
      "NULL"
    } else {
  ...
  de_chunk <- c("### 4. Differential expression", "", de_chunks)
  ...
  c(header, load_chunk, filter_chunk, batch_chunk, de_chunk,
