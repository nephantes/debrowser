...
getConditionSelectorFromMeta <- function(input = NULL, index = 1, num=0, choices = NULL, selected = NULL) {
    ...
    if (nc >= 0) {
        ...
        lapply(seq_len(nc), function(i) {
            ...
            list(column(12, getMetaSelector(input = input, n = i),
                        ...
                        conditionalPanel((condition <- paste0("input.conditions_from_meta",i," != 'No Selection'")),
                                         getConditionSelectorFromMeta(input, i, (2 * i - 1), 
                                                              allsamples, selected1),
                                         getConditionSelectorFromMeta(input, i, (2 * i), 
                                                                      allsamples, selected1)
                                         )
...
getMetaSelector <- function(input = NULL, n = 0){		
    ...
    if(!is.null(metaFile)){		
        ...
        list(HTML('<hr style="color: white; border:solid 1px white;">'), br(), column(10, selectInput(paste0("conditions_from_meta", n),
                                   label = "Select Meta",
...
get_conditions_given_selection <- function(input = NULL, selection){		
    ...
    if(!is.null(metaFile)){		
        ...
        if(length(levels(factor(df[,selection]))) != 2){		
            print("There must be exactly 2 groups.")		
            return(NULL)		
        } else {		
        ...
        print("Meta file does not exist. Please start over and upload one.")		
        return(NULL)		
    }		
}