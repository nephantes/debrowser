    ...
    if (selected_meta != "No Selection") {
      ...
      if (length(grps) == -1) {
        ...
        if (!is.null(selected_meta)) {
          meta_choices_all <- get_conditions_given_selection(
            metadata,
            selected_meta
          )
        }
  ...
  if (nc >= 0) {
    ...
    lapply(seq_len(nc), function(i) {
      ...
      if (!is.null(selectedInput(
        ...
        facts <- levels(factor(metadata[, selectedInput(
          "conditions_from_meta",
          i, NULL, input
        )]))
      ...
      if (!is.null(selectedInput("conditions_from_meta", i, NULL, input)) && !is.null(selectedInput("covariate", i, NULL, input)) &&
        ...
        if (selectedInput("conditions_from_meta", i, NULL, input) %in% selectedInput("covariate", i, NULL, input)) {
          ...
          updateSelectInput(session, paste0("covariate", i),
            choices = c(metadata_columns),
    ...
    withProgress(message = "Running DE Algorithms", detail = inputconds$demethod_params[i], value = 0, {
      ...
      if (!is.null(initd$dat()) && nrow(initd$dat()) > 1) {
        ...
        if (meta_selection != "No Selection") {
          ...
          cond_names <- c(
            unique(ctable$cond_name[ctable$conds == paste0("Cond", 2 * i - 1)]),
            unique(ctable$cond_name[ctable$conds == paste0("Cond", 2 * i)])
          )
        }
        inputconds$dclist[[i]] <- list(
          conds = conds, cols = cols, cond_names = cond_names, init_data = initd$dat(),
